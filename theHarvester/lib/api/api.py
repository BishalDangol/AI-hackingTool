...
async def root(*, user_agent: Annotated[str | None, Header()] = None) -> Response:
    """Root endpoint that serves the passive domain monitoring dashboard HTML."""
    ...
    try:
        static_file_path = 'theHarvester/lib/api/static/index.html'
        if os.path.exists(static_file_path):
            with open(static_file_path, encoding='utf-8') as f:
                return HTMLResponse(f.read())
    except Exception as e:
        print(f"Error serving dashboard homepage: {e}")
    ...
    <html lang="en-US">
        ...
        <body>
            <h3>Dashboard resources loading error or missing index.html.</h3>
