...
function DocumentationPage() {
  return <div className="docs-page"><div className="docs-topbar"><a className="docs-brand" href="/"><span className="brand-mark">H</span><span><b>Harvester</b><small>Security workspace</small></span></a><a className="button secondary small" href="/">← Back to workspace</a></div><div className="docs-layout"><aside className="docs-toc card-surface"><span className="overline">Documentation</span><h2>Scan guide</h2><a href="#how-scan">How scanning works</a><a href="#how-to-scan">How to scan</a><a href="#scan-options">Scan options</a><a href="#results">Understanding results</a><a href="#problems">If something fails</a><a href="#safe-use">Safe use</a></aside><article className="docs-content"><div className="docs-hero"><span className="overline">Simple operator guide</span><h1>How scanning works. How to scan.</h1><p>Use this page as the normal operating guide for the local OSINT web application. Start the backend, open the workspace, run an authorized passive collection, and review the captured evidence.</p><div className="docs-hero-actions"><a className="button primary" href="#how-to-scan">How to scan ↓</a><span className="docs-scope"><span className="scope-dot" />Authorized passive collection only</span></div></div><section id="how-scan" className="docs-section"><SectionTitle number="01" title="How scanning works" text="The browser controls the job, while FastAPI runs and captures theHarvester locally." /><div className="docs-flow">{['Browser sends target and options', 'Backend validates the request', 'theHarvester runs locally', 'Output is captured line by line', 'WebSocket streams live output', 'REST polling finishes the job if needed'].map((step, index) => <div className="docs-flow-step" key={step}><b>0{index + 1}</b><span>{step}</span></div>)}</div><div className="callout"><b>What the system does:</b> it builds a safe argument list, runs the selected passive sources, stores stdout and stderr, and turns the captured report into entities, findings, charts, and a downloadable report.</div></section><section id="how-to-scan" className="docs-section"><SectionTitle number="02" title="How to scan" text="Follow these steps for a normal scan from the browser." /><div className="docs-numbered"><DocStep number="1" title="Start the backend" text="Open Git Bash in the project root, activate the virtual environment, and start FastAPI." code={`cd /e/path/to/AI-hackingTool
source .venv/Scripts/activate
python -m uvicorn backend.main:app --host 127.0.0.1 --port 8000 --reload`} /><DocStep number="2" title="Start the frontend" text="Open a second Git Bash window and start the Vite development server." code={`cd /e/path/to/AI-hackingTool/frontend
npm install
npm run dev`} /><DocStep number="3" title="Open the workspace" text="Visit the Vite address, normally http://localhost:5173, and open Domain OSINT." /><DocStep number="4" title="Enter an authorized target" text="Enter a domain that you own or are explicitly authorized to assess, such as authorized.example.com." /><DocStep number="5" title="Choose sources and options" text="Start with Recommended sources and a limit of 50 or 100. Enable DNS brute force only when it is appropriate for the approved scope." /><DocStep number="6" title="Start and review" text="Click Start passive collection. Watch the live output, wait for Finished, then review entities, services, findings, and the report." /></div></section><section id="scan-options" className="docs-section"><SectionTitle number="03" title="Scan options" text="Use conservative settings first, then expand only when necessary." /><div className="docs-table"><div><b>Option</b><b>Meaning</b></div><div><span>Target domain</span><span>The authorized domain passed to theHarvester.</span></div><div><span>Sources</span><span>Whitelisted passive providers selected for this job.</span></div><div><span>Result limit</span><span>Maximum result count requested from theHarvester, from 1 to 5000.</span></div><div><span>DNS brute force</span><span>Optional subdomain enumeration flag. Use only within the approved scope.</span></div><div><span>Shodan</span><span>Passive host metadata such as ports and banners for authorized assets. It does not open services or camera feeds.</span></div></div></section><section id="results" className="docs-section"><SectionTitle number="04" title="Understanding results" text="The UI shows only information found in the captured report." /><div className="docs-grid"><DocCard title="Collected intelligence" eyebrow="Evidence"><p><b>Emails, hosts, IPs, URLs, people, and services</b> are deduplicated from the actual output. A count of zero means no matching value was captured from the selected providers.</p></DocCard><DocCard title="Findings" eyebrow="Review"><p>Warnings such as missing provider keys, unexpected responses, empty results, and transport fallback are shown with evidence and remediation guidance.</p></DocCard></div><div className="callout"><b>Provider result versus system error:</b> a message such as “0 hosts, 0 IPs, 0 emails” is a provider result. A process error, missing executable, or 15-minute timeout is a system issue.</div></section><section id="problems" className="docs-section"><SectionTitle number="05" title="If something fails" text="Check the process that owns the failing part." /><div className="docs-module-list"><DocRow title="Backend offline" text="Run curl http://127.0.0.1:8000/api/health and restart Uvicorn if it does not return a healthy response." /><DocRow title="WebSocket warning" text="Activate the project .venv and install websockets==15.0.1. The UI will use REST polling if live streaming is unavailable." /><DocRow title="Polling timeout" text="Pull the latest main branch and restart both processes. Current polling matches the backend’s 15-minute passive-job limit." /><DocRow title="Missing provider key" text="Configure only the provider credentials you need. Keep keys in environment variables or ignored local files." /><DocRow title="No entities" text="Review the terminal for provider errors and try fewer recommended passive sources with a smaller limit." /></div></section><section id="safe-use" className="docs-section docs-last"><SectionTitle number="06" title="Safe use" text="This workspace is for authorized passive assessment and remediation reporting." /><div className="callout safety-callout"><b>Do not</b> access camera feeds, guess credentials, bypass authentication, execute exploits, perform active port scans, or assess assets without permission. Never commit API keys or include them in screenshots and bug reports.</div></section></article></div></div>;
}

function DocStep({ number, title, text, code }) { return <div className="docs-step"><span className="docs-step-number">{number}</span><div><h3>{title}</h3><p>{text}</p>{code && <CodeBlock>{code}</CodeBlock>}</div></div>; }

function SectionTitle({ number, title, text }) { return <div className="docs-section-title"><span className="docs-number">{number}</span><div><h2>{title}</h2><p>{text}</p></div></div>; }
function DocCard({ title, eyebrow, children }) { return <div className="docs-card card-surface"><span className="overline">{eyebrow}</span><h3>{title}</h3>{children}</div>; }
function DocRow({ title, text }) { return <div className="docs-row"><span className="docs-row-mark">→</span><div><b>{title}</b><p>{text}</p></div></div>; }
function CodeBlock({ children }) { return <pre className="docs-code"><code>{children}</code></pre>; }

function App() {
  const isDocs = typeof window !== 'undefined' && window.location.pathname === '/doc';
  ...
  const [shodanReview, setShodanReview] = useState(null);
  ...
  async function runShodanReview() {
    if (!shodanAuthorized || !domain.trim()) return;
    setShodanReview({ status: 'loading', asset: domain.trim() });
    try {
      const response = await fetch(`${API}/api/modules/shodan-assets/review`, { method: 'POST', headers: { 'Content-Type': 'application/json' }, body: JSON.stringify({ asset: domain.trim(), authorized: true, limit: 10 }) });
      const data = await response.json();
      if (!response.ok) throw new Error(data.detail || `Shodan review returned ${response.status}`);
      setShodanReview(data);
    } catch (error) { setShodanReview({ status: 'error', asset: domain.trim(), message: error.message }); }
  }
  ...
  return <div className="app-shell">
    ...
    <main className="main-shell"><header className="topbar"><div className="breadcrumb">Harvester <span>/</span> {moduleTitle}</div><div className={apiOnline ? 'connection-pill online' : 'connection-pill'}><span />{apiStatus}</div></header>
      ...
      {module === 'shodan' && <ShodanModule target={domain} setTarget={setDomain} authorized={apiOnline && shodanAuthorized} setAuthorized={setShodanAuthorized} onReview={runShodanReview} review={shodanReview} diagnostics={diagnostics} />}
...
function ShodanModule({ target, setTarget, authorized, setAuthorized, onReview, review, diagnostics }) {
  const assets = review?.assets || [];
  return <section className="module-page"><ModuleHeader overline="Shodan asset intelligence" title="One passive result for an approved target." text="Resolve the target, correlate Shodan-indexed metadata, and review exposed service evidence without connecting to the target." /><div className="module-grid"><div className="card-surface tool-card"><span className="tool-number">01</span><h3>Authorized target</h3><p>Use a domain or IP address belonging to your organization or approved assessment scope.</p><label className="field-label">Domain or IP address</label><input className="field-input" value={target} onChange={event => setTarget(event.target.value)} placeholder="authorized.example.com" /><label className="confirm-row"><input type="checkbox" checked={authorized} onChange={event => setAuthorized(event.target.checked)} /><span>I confirm this target is owned by or authorized for assessment.</span></label><button className="button primary" disabled={!authorized || !target.trim() || review?.status === 'loading'} onClick={onReview}>{review?.status === 'loading' ? 'Reviewing passive data…' : 'Run Shodan asset review →'}</button><p className="small-note">Key status: {diagnostics?.shodan_key_configured ? 'configured' : 'not detected'}</p></div><div className="card-surface checklist-card"><span className="tool-number">02</span><h3>What is reviewed</h3><Check text="Resolved IP addresses" /><Check text="Indexed hostnames and domains" /><Check text="Ports, products, and service banners" /><Check text="Organization, ASN, ISP, and last update" /><p className="small-note">Passive metadata only. No service connections, login attempts, active scans, or camera-feed access.</p></div></div>{review?.status === 'error' && <div className="alert-card"><span>!</span><div><b>Shodan review could not complete</b><p>{review.message}</p></div></div>}{review?.status === 'completed' && <div className="shodan-result card-surface"><div className="result-header"><div><span className="overline">03 · Passive result</span><h2>{review.asset}</h2><p className="small-note">{review.resolved_ips?.length || 0} resolved IPs · {review.queried_count || 0} indexed records reviewed</p></div><span className="status-tag finished">Completed</span></div><div className="shodan-summary"><Metric label="Resolved IPs" value={review.resolved_ips?.length || 0} note="DNS resolution" /><Metric label="Records" value={review.queried_count || 0} note="Shodan responses" /><Metric label="Services" value={new Set(assets.flatMap(item => item.services || [])).size} note="Indexed labels" /></div><div className="shodan-assets-list">{assets.length ? assets.map(asset => <article className="shodan-asset-row" key={asset.ip}><div className="shodan-asset-head"><strong>{asset.ip}</strong><span>{asset.status === 'not_indexed' ? 'Not indexed' : asset.organization || asset.asn || 'Indexed asset'}</span></div>{asset.status === 'not_indexed' ? <p className="small-note">No indexed Shodan metadata was returned for this address.</p> : <div className="shodan-asset-details"><span><b>Hosts</b>{(asset.hostnames || []).join(', ') || 'None reported'}</span><span><b>Ports</b>{(asset.ports || []).join(', ') || 'None reported'}</span><span><b>Services</b>{(asset.services || []).join(', ') || 'None reported'}</span><span><b>Network</b>{[asset.isp, asset.asn].filter(Boolean).join(' · ') || 'Not reported'}</span></div>}</article>) : <p className="empty-state">No resolved addresses were available for this target.</p>}</div><div className="callout"><b>Interpretation:</b> this is an indexed passive snapshot, not proof that a service is currently reachable or vulnerable. Verify ownership and remediate exposed services through your approved infrastructure process.</div></div>}</section>;
}
