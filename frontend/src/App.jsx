...
function DocumentationPage() {
  return <div className="docs-page"><div className="docs-topbar"><a className="docs-brand" href="/"><span className="brand-mark">H</span><span><b>Harvester</b><small>Security workspace</small></span></a><a className="button secondary small" href="/">← Back to workspace</a></div><div className="docs-layout"><aside className="docs-toc card-surface"><span className="overline">Documentation</span><h2>Operator guide</h2><a href="#start">Quick start</a><a href="#workflow">How scanning works</a><a href="#modules">Modules</a><a href="#results">Reading results</a><a href="#api">API reference</a><a href="#troubleshooting">Troubleshooting</a><a href="#safety">Safe use</a></aside><article className="docs-content"><div className="docs-hero"><span className="overline">Local operator documentation</span><h1>Run, understand, and review every collection.</h1><p>This guide explains how to run the web application on Windows, configure the local backend, use each workspace module, and interpret captured passive OSINT evidence.</p><div className="docs-hero-actions"><a className="button primary" href="#start">Start here ↓</a><span className="docs-scope"><span className="scope-dot" />Authorized passive assessment only</span></div></div><section id="start" className="docs-section"><SectionTitle number="01" title="Quick start" text="Start the backend and frontend in two separate Git Bash windows." /><div className="docs-grid"><DocCard title="Backend" eyebrow="Terminal 01"><CodeBlock>{`cd /e/path/to/AI-hackingTool
source .venv/Scripts/activate
python -m pip install -e .
export SHODAN_API_KEY='your-local-key'
python -m uvicorn backend.main:app --host 127.0.0.1 --port 8000 --reload`}</CodeBlock><p>Check <code>http://127.0.0.1:8000/api/health</code> before opening the UI.</p></DocCard><DocCard title="Frontend" eyebrow="Terminal 02"><CodeBlock>{`cd /e/path/to/AI-hackingTool/frontend
npm install
npm run dev`}</CodeBlock><p>Open the Vite address, normally <code>http://localhost:5173</code>.</p></DocCard></div></section><section id="workflow" className="docs-section"><SectionTitle number="02" title="How scanning works" text="The application captures theHarvester output without shell execution and keeps the browser updated." /><div className="docs-flow">{['Enter an authorized target', 'Choose passive sources', 'Backend validates input', 'theHarvester runs locally', 'WebSocket or REST captures output', 'Entities and findings are summarized'].map((step, index) => <div className="docs-flow-step" key={step}><b>0{index + 1}</b><span>{step}</span></div>)}</div><div className="callout"><b>Important:</b> a provider returning zero results is not automatically a system failure. Review the live output for missing keys, provider errors, and parser diagnostics.</div></section><section id="modules" className="docs-section"><SectionTitle number="03" title="Workspace modules" text="Use the module that matches the evidence you need." /><div className="docs-module-list"><DocRow title="Domain OSINT" text="Run theHarvester against an authorized domain and collect emails, hosts, IPs, URLs, people, and service labels." /><DocRow title="DNS & Certificates" text="Review passive DNS and certificate-transparency signals, including certificate names and unexpected subdomains." /><DocRow title="Shodan Assets" text="Review passive host metadata such as ports, banners, products, organization, and ASN after confirming asset authorization." /><DocRow title="Camera Audit" text="Record hardening checks for cameras in your own inventory. The module never opens feeds or attempts access." /><DocRow title="Findings" text="Turn captured provider warnings, empty results, and transport issues into evidence-based remediation cards." /><DocRow title="Reports" text="Review in-memory jobs and export the current captured report as a text file." /></div></section><section id="results" className="docs-section"><SectionTitle number="04" title="Reading results" text="The result panels are derived from captured output only." /><div className="docs-table"><div><b>Panel</b><b>What it shows</b></div><div><span>Emails</span><span>Deduplicated email addresses found in the report.</span></div><div><span>Hosts</span><span>Domains and subdomains detected in captured lines.</span></div><div><span>IP addresses</span><span>IPv4 addresses detected in the report.</span></div><div><span>Services</span><span>Passive service and port labels such as HTTPS, SSH, or port 443.</span></div><div><span>Findings</span><span>Diagnostics with severity, evidence, and recommended remediation.</span></div></div></section><section id="api" className="docs-section"><SectionTitle number="05" title="Useful API endpoints" text="Use these endpoints to troubleshoot or integrate another local client." /><div className="endpoint-list"><code>GET /api/health</code><code>GET /api/diagnostics</code><code>POST /api/scan</code><code>GET /api/scan/{'{job_id}'}/result</code><code>GET /api/scan/{'{job_id}'}/download</code><code>GET /api/runs/{'{run_id}'}/report</code></div></section><section id="troubleshooting" className="docs-section"><SectionTitle number="06" title="Troubleshooting" text="Most startup issues are caused by using a different environment than the one running Uvicorn." /><div className="docs-grid"><DocCard title="WebSocket warning" eyebrow="Live output"><p>Activate <code>.venv</code> and install <code>websockets==15.0.1</code>. The frontend also falls back to REST polling.</p></DocCard><DocCard title="Polling timeout" eyebrow="Long scans"><p>Pull the latest code and restart both processes. The polling window matches the backend’s 15-minute job limit.</p></DocCard></div></section><section id="safety" className="docs-section docs-last"><SectionTitle number="07" title="Safe operating boundary" text="Use this application for authorized, passive assessment and remediation reporting." /><div className="callout safety-callout"><b>Never</b> use the application to access camera feeds, guess credentials, bypass authentication, execute exploits, perform active port scans, or assess assets without permission. Keep API keys out of Git, screenshots, and bug reports.</div></section></article></div></div>;
}
